## ClassicCreamery API Notes

In the `docs/json_samples` directory, you will find sample JSON generated for the API endpoints for your application. The only endpoints we are requiring you to create for this phase are:

- http://localhost:3000/v1/employees
- http://localhost:3000/v1/shifts/{id}.json
- http://localhost:3000/v1/stores/{id}/upcoming
- http://localhost:3000/v1/spotlight/{id}.json

All are read-only routes passed as a GET request.

## General notes in building this API:

1. We have already given you the appropriate `ApiController` needed to build your controllers for these endpoints. In addition, we've given you a separate folder to store these api controllers in (`app/controllers/api/v1`). Finally, we have given you some working endpoints that are needed for the React portion of the project and can be studied as examples.
2. We have already installed the `jsonapi-serializer` gem in your `Gemfile` -- you simply need to run `bundle install` to get it.
...
5. We have already included controller tests for all controllers, including both the api controllers and our regular controllers generating html.
6. Documentation for the API using Swagger Docs is optional and will not be tested or graded.
