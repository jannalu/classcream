...
class Ability
  ...
  def initialize(employee)
    ...
    ## ADMIN
    if employee.admin_role? #|| employee.manager_role?
    ...
    if employee.employee_role?
      ...
      # can access the time clock (including React API actions)
      can [:time_clock, :time_in, :time_out, :current_shift, :clock_in, :clock_out], Shift
    end
